		...
		return apply_filters( 'submit_event_form_fields', array(
			...
			'dj' => array(
				'event_dj_ids' => array(
					...
					'type'      		=> 'multiselect',
					'default'   		=> $default_dj,
					'options'   		=> apply_filters('wpem_set_dj_ids', ($current_user_id) ? (function() {
						$args = array(
							'post_type'      => 'event_dj',
							'posts_per_page' => -1,
							'orderby'        => 'title',
			...
			'local' => array(
				'event_local_ids' => array(
				    ...
				    'options'  		=> apply_filters('wpem_set_local_ids', ($current_user_id) ? (function() {
			            $args = array(
			            ...
			            $options = ['' => __( 'Select Local', 'wp-event-manager')];
			            foreach ($locals as $local) {
			                $options[$local->ID] = $local->post_title;
			            }
			            return $options;
			        })() : ['' => __( 'Select Local', 'wp-event-manager')]),
			...
			switch ( $allowed_registration_method ) {
				case 'email' :
					if( !is_email( $values['event']['registration'] ) ) {
						throw new Exception( esc_attr( __( 'Please enter a valid registration email address.', 'wp-event-manager' ) ) );
					}
				break;
				case 'url' :
					...
					if( !filter_var( $values['event']['registration'], FILTER_VALIDATE_URL ) ) {
						throw new Exception( esc_attr( __( 'Please enter a valid registration URL.', 'wp-event-manager' ) ) );
					}
				break;
				default :
					if( !is_email( $values['event']['registration'] ) ) {
